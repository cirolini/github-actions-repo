...
| 7. Deploy | job `publicar`, com aprovação manual |
...
O `cd.yml` tem dois jobs. O `empacotar` roda sozinho; o `publicar` depende de um
*environment* protegido chamado `producao`.

Para ativar a aprovação manual: **Settings → Environments → New environment →
`producao` → Required reviewers**.
...
Verificadas em agosto de 2026: `actions/checkout@v6`, `actions/setup-python@v6`
e Python 3.14. Essas versões mudam com frequência — a fonte a consultar é a
página de *releases* de cada action.
